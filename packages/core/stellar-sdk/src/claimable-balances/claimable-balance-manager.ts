...
export class ClaimableBalanceManager {
  ...
  async createClaimableBalance(
    ...
    try {
      ...
      throw new Error(
        `Failed to create claimable balance: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async claimBalance(
    ...
    try {
      ...
      throw new Error(
        `Failed to claim balance: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
   ...
   */
  async getBalanceDetails(balanceId: string): Promise<ClaimableBalance> {
    try {
      ...
      throw new Error(
        `Failed to get balance details: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getClaimableBalances(
    ...
    try {
      ...
      return response.records.map((balance) =>
        this.mapHorizonBalanceToClaimableBalance(balance)
      );
    } catch (error) {
      throw new Error(
        `Failed to query claimable balances: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
   ...
   */
  private validateCreateParams(params: CreateClaimableBalanceParams): void {
    if (!params.asset) {
    ...
    params.claimants.forEach((claimant, index) => {
      ...
      try {
        ...
        throw new Error(
          `Invalid predicate for claimant at index ${index}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    });
  }
   ...
   * @returns boolean
   */
  private isValidBalanceId(balanceId: string): boolean {
   ...
   */
  private async extractBalanceId(result: any): Promise<string> {
    try {
      ...
      throw new Error(
        'Could not extract balance ID from transaction result. Please query the operation to get the balance ID.'
      );
    } catch (error) {
      throw new Error(
        `Failed to extract balance ID: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private mapHorizonPredicateToPredicate(
    ...
    if (horizonPredicate.abs_before) {
      const timestamp = new Date(
        parseInt(horizonPredicate.abs_before, 10) * 1000
      );
