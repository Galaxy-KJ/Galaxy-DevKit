...
import { PasswordStrength, } from '../types/wallet.types.js';
...
(async () => {
    try {
        ...
        console.error('CRITICAL: argon2 native module failed to load. New encryptions will be rejected. Decryption of existing keys will use PBKDF2 fallback.');
    }
})();
/**
...
export async function encryptPrivateKey(privateKey, password) {
    ...
    if (ENCRYPTION_V2_ENABLED) {
        if (!argon2Available || !argon2Module) {
            throw new Error('Argon2id unavailable — cannot create secure encryption. Install argon2 native module.');
        }
    ...
    if (ENCRYPTION_V2_ENABLED) {
        const argon2Params = Buffer.from(JSON.stringify({ m: ARGON2_MEMORY_COST, t: ARGON2_TIME_COST, p: ARGON2_PARALLELISM })).toString('base64');
        return [
            'v2',
...
export async function decryptPrivateKey(encryptedData, password) {
    ...
    if (isV2) {
        ...
        if (argon2Available && argon2Module) {
            ...
            console.warn('WARNING: Argon2 unavailable, using PBKDF2 degraded fallback for v2 decryption. Key access preserved but KDF strength reduced.');
            key = crypto.pbkdf2Sync(password, salt, DEFAULT_ITERATIONS, KEY_LENGTH, 'sha256');
        }
 ...
 */
export async function decryptPrivateKeyToString(encryptedData, password) {
 ...
 */
export async function withDecryptedKey(encryptedKey, password, callback) {
 ...
 */
export async function encryptData(data, password, params) {
    const salt = params?.salt || crypto.randomBytes(SALT_LENGTH);
    const iv = crypto.randomBytes(IV_LENGTH);
    const iterations = params?.iterations || DEFAULT_ITERATIONS;
    const key = crypto.pbkdf2Sync(password, salt, iterations, params?.keyLength || KEY_LENGTH, params?.digest || 'sha256');
    const cipher = crypto.createCipheriv(ALGO, key, iv);
    const encrypted = Buffer.concat([
 ...
 */
export async function decryptData(encryptedData, password, params) {
...
export function hashPassword(password, salt) {
    ...
    const hash = crypto.pbkdf2Sync(password, saltBuffer, DEFAULT_ITERATIONS, KEY_LENGTH, 'sha256');
...
export function verifyPassword(password, hash) {
    ...
    const testHash = crypto.pbkdf2Sync(password, salt, DEFAULT_ITERATIONS, KEY_LENGTH, 'sha256');
 ...
 * @returns Derived key
 */
export function deriveKey(password, salt, iterations = DEFAULT_ITERATIONS) {
