...
export function getCurrentNetworkConfig(): { rpcUrl: string; networkPassphrase: string; type: NetworkType } {
  const config = networkStore.getConfig();
  return {
    rpcUrl: config.rpcUrl,
    networkPassphrase: config.passphrase,
    type: networkStore.getNetwork(),
  };
}

export function setSelectedNetwork(network: NetworkType): void {
  networkStore.switch(network);
}
