...
export const supplyCommand = new Command('supply')
    ...
    .option('-a, --asset <code>', 'Asset code (e.g., XLM, USDC)', 'XLM')
    .option('-i, --issuer <address>', 'Asset issuer address (not needed for XLM)')
    .option('--amount <amount>', 'Amount to supply')
    ...
    .action(async (options: any) => {
        ...
        try {
            ...
            if (!amount) {
                ...
                const answers = await inquirer.prompt([
                    {
                        type: 'input',
                        name: 'amount',
                        message: 'Enter amount to supply:',
                        validate: (input) => {
                ...
                spinner.start('Supplying to Blend...');
            }
            ...
            const asset = assetCode === 'XLM'
                ...
                : {
                    code: options.asset.toUpperCase(),
                    issuer: options.issuer || '',
                    type: 'credit_alphanum4' as const
                };

            // 7. Execute supply
            spinner.text = `Supplying ${amount} ${asset.code} to Blend...`;
            ...
            spinner.succeed(chalk.green('✅ Supply successful!'));
            ...
            spinner.fail(chalk.red('Supply failed'));
            if (options.json) {
